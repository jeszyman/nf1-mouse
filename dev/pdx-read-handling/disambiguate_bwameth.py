...
# Run: conda run -n biotools python3 disambiguate_bwameth.py --human-bam ... --mouse-bam ... --out-prefix ...
"""Disambiguate PDX reads by comparing NM tags from dual bwa-meth alignments.

Variant of disambiguate.py adapted for bwa-meth output. bwa-meth converts
both reads and reference to a C->T-collapsed alphabet before running bwa mem,
so the NM tag reported is bwa mem's edit distance in conversion space:
bisulfite conversion noise is invisible, and NM reflects true mismatches plus
any indel / soft-clip penalties.

This variant relies on the fact that both species alignments apply the same
c2t transformation, so relative NM comparison between human and mouse BAMs
is well-defined for disambiguation. There is no ZC tag and none is required.

Caveats (vs the BISCUIT-validated disambiguate.py):
  - bwa-meth NM is post-conversion only; the script never sees per-position
    conversion vs true-mismatch separation that BISCUIT exposes via ZC.
  - bwa-mem's NM penalizes indels and soft-clipped boundaries; tie-breaking
    behavior on multi-mapping reads has not been validated against BISCUIT.
  - Use the BISCUIT variant if you need methylation-aware disambiguation
    for downstream methylation calling on the disambiguated BAM.
